...
export function createAgentToken(token: string, label: string): void {
  db.prepare("INSERT INTO agent_tokens (token, label, created_at) VALUES (?, ?, ?)").run(
    token,
    label,
    Date.now(),
  );
...
export function getAgentToken(token: string): { token: string; label: string } | undefined {
  return db.prepare("SELECT token, label FROM agent_tokens WHERE token = ?").get(token) as
    | { token: string; label: string }
    | undefined;
}

export function touchAgentToken(token: string): void {
  db.prepare("UPDATE agent_tokens SET last_seen_at = ? WHERE token = ?").run(Date.now(), token);
}

export function listAgentTokens(): { token: string; label: string; created_at: number; last_seen_at: number | null }[] {
  return db
    .prepare("SELECT token, label, created_at, last_seen_at FROM agent_tokens ORDER BY created_at DESC")
    .all() as any[];
}
